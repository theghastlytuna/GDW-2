...
void BarBreakerListener::BeginContact(b2Contact* contact)
{
	...
	if ((filterA.categoryBits == PICKUP) || (filterB.categoryBits == PICKUP))
	{
		...
		if (filterA.categoryBits == PLAYER)
		{
			...
			{
				ECS::GetComponent<PhysicsBody>((int)fixtureA->GetBody()->GetUserData()).GetBody()->ApplyLinearImpulseToCenter(b2Vec2(-70000.f, 70000.f), true);
		...
		else if (filterB.categoryBits == PLAYER)
		{
			...
			{
				ECS::GetComponent<PhysicsBody>((int)fixtureB->GetBody()->GetUserData()).GetBody()->ApplyLinearImpulseToCenter(b2Vec2(-70000.f, 70000.f), true);
